...
#grid(
  ...
  box(
    ...
    radius: 999pt,
    inset: (x: 12pt, y: 8pt),
    text(size: 0.9em, weight: "bold", fill: white)[Fiche n°9]
  ),
)
  ...
  box(
    ...
    stack(dir: ttb, spacing: 0.35em,
      ...
      stack(dir: ttb, spacing: 0.1em,
        ...
        text(size: 0.85em)[Comparer les captages selon leur vulnérabilité intrinsèque liée à leur nature {un captage superficiel est considéré comme plus vulnérable qu'un forage profond dans les calculs de criticité)],
      ),
      stack(dir: ttb, spacing: 0.1em,
      ...
      stack(dir: ttb, spacing: 0.1em,
        ...
        text(size: 0.85em)[Négatif (superficiel = critique) #h(0.2em) #text(fill: rgb("#c62828"), size: 1.1em)[↓]],
      ),
      stack(dir: ttb, spacing: 0.1em,
        text(size: 0.85em, weight: "bold")[Définition de la criticité],
        text(size: 0.85em)[Captage superficiel = plus vulnérable → criticité forte],
      ),
    )
  ),
  box(
