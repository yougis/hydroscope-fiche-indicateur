...
#grid(
  ...
  [
    #box(
      ...
      stroke: 1.2pt + brand,
      [
        #text(size: 0.85em, weight: "bold", fill: brand)[FICHE 7 <ind-7>]
      ]
    )
  ]
)
...
#stack(dir: ltr, spacing: 0.6em,
  ...
  box(
    ...
    inset: (x: 6pt, y: 4pt),
    radius: 3pt,
    text(size: 0.75em, weight: "medium", fill: rgb("#475569"))[Sécurité sanitaire et règlementaire]
  )
)
#v(0.6em)
#box(
  ...
  stack(dir: ttb, spacing: 0.4em,
    text(size: 0.73em, style: "italic", fill: rgb("#334155"))[Définit le type de traitement d'eau associé à la ressource en eau avant sa mise en distribution. La donnée est issue des fichiers de l'unité de distribution de la DASS, qui décrivent le fonctionnement technique et sanitaire des installations de traitement.],
    1em,
    text(size: 0.71em, weight: "bold", fill: rgb("#1a365d"))[Objectif :],
    text(size: 0.71em, fill: rgb("#1a202c"))[Informer sur le niveau de traitement appliqué à l’eau avant distribution afin de caractériser la sécurisation sanitaire du captage.],
  )
)
#v(0.5em)
#grid(
  ...
  box(
    ...
    text(size: 0.71em, fill: rgb("#1a202c"))[Évaluer les risques liés à la qualité de la ressource en eau, une absence ou insuffisance de traitement augmentant la criticité sanitaire.],
      0.7em,
      stack(dir: ttb, spacing: 0.4em,
        text(size: 0.75em, weight: "bold")[Modalités de traitement],
        text(size: 0.71em)[Attribution d’une classe standardisée par unité de distribution, puis association au captage.],
      ),
      0.7em,
      stack(dir: ttb, spacing: 0.4em,
  ...
  stack(dir: ttb, spacing: 0pt,
   block(breakable: true,
      ...
      stack(dir: ttb, spacing: 0.4em,
        ...
        v(0.4em),
        text(size: 0.72em)[#text(fill: rgb("#475569"), weight: "medium")[Unité :] Classe \(pas de traitement \/ traitement partiel \/ traitement complet\)],
