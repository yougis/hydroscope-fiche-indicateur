...
#grid(
  ...
  [
    #box(
      ...
      stroke: 1.2pt + brand,
      [
        #text(size: 0.85em, weight: "bold", fill: brand)[FICHE 6 <ind-6>]
      ]
    )
  ]
)
...
#stack(dir: ltr, spacing: 0.6em,
  ...
  box(
    ...
    inset: (x: 6pt, y: 4pt),
    radius: 3pt,
    text(size: 0.75em, weight: "medium", fill: rgb("#475569"))[Niveau infrastructures]
  )
)
#v(0.6em)
#box(
  ...
  stack(dir: ttb, spacing: 0.4em,
    text(size: 0.73em, style: "italic", fill: rgb("#334155"))[Établit le volume de stockage des réservoirs d'eau de l'unité de distribution \(UD\) associée à la ressource. Cet indicateur mobilise les informations administratives internes de la DASS relatives aux capacités de stockage d'eau potable des unités de distribution.],
    1em,
    text(size: 0.71em, weight: "bold", fill: rgb("#1a365d"))[Objectif :],
    text(size: 0.71em, fill: rgb("#1a202c"))[Informer sur la capacité de stockage d’eau associée au captage afin de comprendre la capacité de régulation du système.],
  )
)
#v(0.5em)
#grid(
  ...
  box(
    ...
    text(size: 0.71em, fill: rgb("#1a202c"))[Évaluer la résilience de la ressource en eau face aux aléas, un faible stockage augmentant la criticité.],
      0.7em,
      stack(dir: ttb, spacing: 0.4em,
        text(size: 0.75em, weight: "bold")[Modalités de traitement],
        text(size: 0.71em)[Somme des volumes par unité de distribution, puis rattachement au captage.],
      ),
      0.7em,
      stack(dir: ttb, spacing: 0.4em,
